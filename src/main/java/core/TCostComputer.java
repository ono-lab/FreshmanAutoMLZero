...
public class TCostComputer {
  static public double execute(ArrayList<TInstruction> component) {
    double cost = 0.0;
    for (TInstruction instr : component) {
      cost += execute(instr);
    }
    return cost;
  }
  ...
  static public double execute(TInstruction instr) {
    switch (instr.getOp()) {
      case NO_OP:
        return 3013921.0;
      case SCALAR_SUM_OP:
      ...
      case VECTOR_ROW_BROADCAST_OP:
        return 11087322.0;
      // All ops should be supported here.
      default:
